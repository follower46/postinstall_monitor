...
def execute_post_install_script(device_id, device_type="Hardware"):
    """
    Downloads and executes post install script on a device
    """
    login_info = get_device_login_credentials(device_id, device_type)
...
def get_device_login_credentials(device_id, device_type="Hardware"):
    """
...
'''
    ...
    device = ApiClient().get(device_type).getObject(mask=mask, id=device_id)
